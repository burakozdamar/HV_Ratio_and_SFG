...
real*8 :: rC, rO, rCmin, rOmin
real*8 :: NCtop, NOtop, NCtop_Mean, NOtop_Mean
real*8, dimension(:), allocatable :: xC, yC, zC, xO, yO, zO
...
open(3,file="C-3-Gaussian-Map.dat")
...
do t = 1,Nstep/10
    ...
    rCmin = 11.d0
        do j = 1, NIdown
            xdiffC = xC(i) - xIdown(j)
            ydiffC = yC(i) - yIdown(j)
            zdiffC = zC(i) - zIdown(j)
            call pbc(xdiffC, a)
            call pbc(ydiffC, b)
            call pbc(zdiffC, c)
            rC = sqrt(xdiffC**2+ydiffC**2+zdiffC**2)
            if (rC <= rCmin) then 
                rCmin = rC
            endif
        enddo !end j
            if (rCmin <= cutoff) then
                ...
                NCtop = NCtop + 1
                do k = 1, nlx
                    if (xC(i) <= lx2(k) .and. xC(i) >= (lx2(k)-dlx)) then
                        do h = 1, nly
                            if (yC(i) <= (ly2(h)+dly) .and. yC(i) >= ly2(h)) then
